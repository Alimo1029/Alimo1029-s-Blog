...
class C_Tokens:
    # Token验证类
    def addToken(dict_Token):
        # 添加新的Token
        Tokens.update(dict_Token)
        print(Tokens)
    def searchToken(username, U_Token):
        # 搜索是否存在需求Token
        #print(1)
        seh = Tokens.get(username, False)
        #print(seh)
        if (seh != False):
            if (seh == U_Token):
                return True
    ...
    def delToken(username):
        # 删除已验证Token
        state = Tokens.pop(username, False)
        if (state == False):
            return False
        elif (state != False):
            return True
...
def dataHandler(need, sk):
    if (need == 1):
        ...
        state = C_Tokens.searchToken(username, U_Token)
        if (state == True):
            #可以注册（验证成功）
            sk.send('True'.encode('utf-8'))
            state = C_Tokens.delToken(username)
            # 若返回False，再次删除，避免硬错误
            if (state == False):
                C_Tokens.delToken(username)
        elif (state == False):
            #不能注册（验证失败）
